...
/**
 * Generates a prompt to create/update README.McpDoc.md files in subdirectories containing source code.
 ...
 * @returns A prompt string that will generate README.McpDoc.md files
 */
function expandGenerateReadmePrompt(args: IGenerateReadmePromptArgs): string {
   ...
   const prompt = `Use the filesystem tool to list all subdirectories of ${rootDirectory}. Ignore any 'node_modules' subdirectories. Then ` +
                 `recursively list the contents of each other subdirectory (apart from any 'node_modules' subdirectories) for ${languages} files. ` +
                 `If the subdirectory contains one or more ${languages} files, call the mcp_documenter tool 'should_regenerate_readme' to see if the README file ` +
                 ...
                 `the summaries and write a concatenated summary into a file named README.McpDoc.md in the same subdirectory, giving an absolute ` +
                 `path to the tool.`;
